...
- **Per-task done bar:** `uv run pytest tests/unit/ -m "not live" -q` green, `uv run ruff check src/ tests/` green, `uv run ruff format src/ tests/` applied, `uv run mypy src/` green. Commit after each task with a conventional-commit message.
